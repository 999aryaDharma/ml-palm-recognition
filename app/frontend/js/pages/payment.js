...
async function processPayment(user) {
  ...
  try {
    ...
    addLog("RESULT", "Payment SUCCESS. Transaction ID: " + (result.transaction_id || "TXN-" + Date.now()));
...
function showReceipt(user, result) {
  ...
  document.getElementById("receipt-txn-id").textContent = result.transaction_id || generateTxnId();
  ...
  document.getElementById("receipt-amount").textContent = formatRupiah(ORDER_TOTAL);
...
function addLog(tag, msg) {
  ...
  line.innerHTML = `
    ...
    <span class="log-tag log-tag--${tag.toLowerCase().replace(/_/g, "-")}">${tag}</span>
